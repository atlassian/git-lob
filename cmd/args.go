...
func ParseCommandLine(opts *util.Options, args []string) (errors []string) {
	...
	for _, arg := range args[1:] {
		...
		if match := valueRegex.FindStringSubmatch(arg); match != nil {
			...
			case "version":
				...
				util.LogConsole("git-lob version", util.Version())
				os.Exit(0)
			case "help":
	...
	if opts.Command == "" && !util.GlobalOptions.HelpRequested {
...
func validateCustomOptions(opts *util.Options, validValueOpts, validBoolOpts []string) (errors []string) {
	validValueSet := util.NewStringSetFromSlice(validValueOpts)
	validBoolSet := util.NewStringSetFromSlice(validBoolOpts)
