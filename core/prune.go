...
func cmdPrune() int {
	...
	if optOnlyUnreferenced {
		...
		shas, err = PruneUnreferenced(util.GlobalOptions.DryRun, pruneCallbackImpl)
		...
		shas, err = PruneOld(util.GlobalOptions.DryRun, optSafeMode, pruneCallbackImpl)
		util.LogConsoleSpinnerFinish("Processing: ")
		if err != nil {
			util.LogErrorf("Prune failed: %v\n", err)
			return 3
...
func cmdPruneShared() int {
	...
	shas, err := PruneSharedStore(util.GlobalOptions.DryRun, pruneCallbackImpl)
	util.LogConsoleSpinnerFinish("Processing: ")
	if err != nil {
		util.LogErrorf("Prune failed: %v\n", err)
		return 3
...
func PostFetchPullPrune() ([]string, error) {
	shas, err := PruneOld(false, util.GlobalOptions.PruneSafeMode, pruneCallbackImpl)
	util.LogConsoleSpinnerFinish("Processing: ")
	return shas, err
}
